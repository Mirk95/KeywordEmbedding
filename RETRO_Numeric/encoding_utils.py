...
def text_to_vec(term, vec_bytes, terms, tokenization_settings):
    ...
    :return: a boolean that indicates, if the vector was inferred and the vector itself in the form vector
    """
...
def num_to_vec_one_hot(num, min_value, max_value, column_vec):
    ...
    :return: vector in the form vector
    """
...
def bucket_to_vec_one_hot(bucket, column_vec):
    ...
    if bucket_valid(bucket):
        ...
        if column_vec is not None:
            vec += column_vec
            vec /= 2
        return apply_normalization(vec)
    else:
        return np.zeros(300, dtype='float32')
...
def bucket_to_vec_one_hot_gaussian(bucket):
    ...
    if bucket_valid(bucket):
        vec = np.zeros(300, dtype='float32')
        for x in range(number_dims):
            vec[x] = gaussian(x * 0.5, bucket * 0.5)  # the factor 0.5 stretches the function
        return apply_normalization(vec)
    else:
        return np.zeros(300, dtype='float32')
...
def bucket_to_vec_unary_random_dim(bucket, column_vec):
    ...
    :return: vector
    """
    if bucket_valid(bucket) and column_vec is not None:
        vec = np.zeros(300, dtype='float32')
        vec[0:bucket + 1] = 1.0
        vec[bucket + 1:number_dims] = -1.0
        random.seed(column_vec.tobytes())
...
def initialize_numeric_word_embeddings(df_vectors):
    """
    ...
    if len(number_embeddings.keys()) == 0:
        ...
        df_vectors['word'] = df_vectors['word'].astype('object')
        result = dict()
        count = 0
        for _, word, vector in df_vectors.itertuples(name=None):
            try:
        ...
        for key in result.keys():
            vectors = list(map(lambda x: np.array(x.split(), dtype='float32'), result[key]))
            final_vec = np.zeros(300, dtype='float32')
            for vec in vectors:
                final_vec += vec
            final_vec /= len(vectors)
            number_embeddings[key] = final_vec
