...
def get_adjacency_vector(size, group_name, index_lookup, data_type, conf):
    ...
    for idx, x in res.items():
        if data_type == 'number':
            if x != '':
                group_elements.append(group_name + '#' + str(int(x)))
            else:
                continue
        else:
...
def fill_adjacency_matrix_relational(size, column_names, index_lookup, group, conf):
    ...
    for (_, text_value1, text_value2) in group_elements.itertuples(name=None):
        if group['data_type'][0] == 'string':
            ...
            if text_value1 != '':
                text_value1 = str(int(text_value1))
            else:
                text_value1 = 'None'
...
def create_adjacency_matrices(term_list, groups, conf):
    ...
    for key in groups:
        for group in groups[key]:
            ...
            print('Process group %s:%s ...' % (key, group['name']), '(size: %d)' % element_count)
            ...
            if group['type'] == 'categorial':
                A_cat[matrix_key] = get_adjacency_vector(size, key, index_lookup, group['data_type'], conf)
                c += A_cat[matrix_key]
